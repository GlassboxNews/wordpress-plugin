 ...
 */
class WP_REST_Glassbox_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			$this->namespace,
			'/' . $this->parent_base . '/(?P<parent>[\d]+)/' . "glassbox",
			array(
				'args'   => array(
					'parent' => array(
				...
				array(
					...
					'permission_callback' => array( $this, 'bypass_permissions_check' ),
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_glassbox_items' ),
					'permission_callback' => array( $this, 'bypass_permissions_check' ),
					'args'                => $this->get_collection_params(),
				),
				'schema' => array( $this, 'get_public_item_schema' ),
	...
	/**
	 * Gets a collection for Glassbox
	 *
	 * @since 4.7.0
	 *
	 * @param WP_REST_Request $request Full details about the request.
	 ...
	 */
	public function get_glassbox_items( $request ) {

		$latest_posts = $this->get_last_in_72_hours();
		...
		$response = array();
		foreach($latest_posts as $parent) {
			...
			// only get revisions for las 3 days
			$query_args[ 'date_query' ] = [ 'column' => 'post_modified_gmt', 'after' => '3 days ago' ];
	...
	public function bypass_permissions_check( $request ) {
		return true;
	}
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( in_array( 'date_gmt', $fields, true ) ) {
			$data['published'] = $this->prepare_date_response( $post->post_date_gmt );
		}
		if ( in_array( 'modified_gmt', $fields, true ) ) {
			$data['modified'] = $this->prepare_date_response( $post->post_modified_gmt );
		}

		if ( in_array( 'title', $fields, true )
		     && in_array( 'content', $fields, true )
		     && in_array( 'parent', $fields, true )
		) {
			$parent = (int) $post->post_parent;
			$data['entry'] = array(
				'title' => get_the_title( $post->ID ),
				'content' => apply_filters( 'the_content', $post->post_content ),
				'id' => get_the_permalink( $parent ),
				'link' => array( rest_url( sprintf( '%s/%s/%d', $this->namespace, $this->parent_base, $parent ) ) ),
			);
		}
